...
class DASE_SAC(object):
    def __init__(self, num_inputs, action_space, args, agent_id):
    ...
    def update_parameters(self, memory, batch_size, updates):
        ...
        state_batch, action_batch, reward_batch, next_state_batch, mask_batch, mean_batch, std_batch, agent_id_batch = memory.sample(batch_size=batch_size)
        ...
        agent_id_batch = torch.FloatTensor(agent_id_batch).to(self.device).unsqueeze(1)

        # Obtain the indices corresponding to self and external experiences
        ext_idx, _ = torch.where(agent_id_batch != self.actor_id)
        self_idx, _ = torch.where(agent_id_batch == self.actor_id)

        # Compute KL-Divergence weights
        try:
            # Off-policy correction for the Stochastic SAC
            if self.policy_type == "Gaussian":
                with torch.no_grad():
                ...
                kl_div = (kl_divergence(batch_distribution, current_distribution) + kl_divergence(current_distribution, batch_distribution)) / 2
                kl_div = torch.exp(-torch.mean(kl_div, dim=1)).view(-1, 1)

                kl_weights = kl_div
                kl_weights[self_idx] = 1
            # Off-policy correction for the Deterministic SAC
            else:
                kl_weights = torch.ones_like(reward_batch)

                with torch.no_grad():
                    current_action, _, _, _, _ = self.actor.sample(state_batch[ext_idx])

                # Compute the difference batch
                diff_action_batch = action_batch[ext_idx] - current_action
        ...
        qf1_loss = torch.sum(kl_weights * F.mse_loss(qf1, next_q_value, reduction='none')) / torch.sum(
            kl_weights)
        qf2_loss = torch.sum(kl_weights * F.mse_loss(qf2, next_q_value, reduction='none')) / torch.sum(
            kl_weights)
        ...
        policy_loss = (kl_weights * ((self.alpha * log_pi) - min_qf_pi)).sum() / torch.sum(kl_weights)
